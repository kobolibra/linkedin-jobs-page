...
function renderTop50(rows){
  ...
  const legend='<text class="top20-legend-title" x="'+(W-54)+'" y="'+(legendY-22)+'" text-anchor="middle">REGION</text>'+['CN','HK','SG'].map((r,i)=>'<circle class="top20-legend-dot" cx="'+(W-54)+'" cy="'+(legendY+i*18)+'" r="3.2" fill="'+colors[r]+'"/><text class="top20-legend-label" x="'+(W-44)+'" y="'+(legendY+3+i*18)+'">'+r+'</text>').join('');
  ...
  const areaCN='M '+center+' '+pts[0].y.toFixed(1)+' L '+pointString('cn')+' L '+center+' '+pts[pts.length-1].y.toFixed(1)+' Z';
  const areaHK='M '+center+' '+pts[0].y.toFixed(1)+' L '+pointString('hk')+' L '+center+' '+pts[pts.length-1].y.toFixed(1)+' Z';
  const areaSG='M '+pointString('hk')+' L '+pointString('sg')+' L '+pts.slice().reverse().map(p=>p.hk.toFixed(1)+','+p.y.toFixed(1)).join(' ')+' Z';
