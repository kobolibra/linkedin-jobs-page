...
fetch("jobs.json",{cache:"no-cache"})
  ...
  .then(data=>{
    ...
    const monthStart=new Date(tMin);monthStart.setHours(0,0,0,0);monthStart.setDate(1);const monthTicks=[];for(let d=new Date(monthStart);d.getTime()<=tMax;d.setMonth(d.getMonth()+1)){const ratio=(d.getTime()-tMin)/tSpan;if(ratio>=-0.02&&ratio<=1.02){const x=left+Math.max(0,Math.min(1,ratio))*plotW;monthTicks.push('<line class="region-time-guide" x1="'+x.toFixed(1)+'" y1="'+(top-11)+'" x2="'+x.toFixed(1)+'" y2="'+(top+rowGap*2+10)+'"/><text class="region-time-label" x="'+x.toFixed(1)+'" y="'+(TH-5)+'">'+escSvg(d.toLocaleDateString('zh-CN',{month:'numeric'}))+'</text>');}}const axisTicks=monthTicks.join('');
