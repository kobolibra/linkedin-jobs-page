...
fetch("jobs.json",{cache:"no-cache"})
  ...
  .then(data=>{
    ...
    const TW=520,TH=126,left=72,right=76,top=27,rowGap=27,plotW=TW-left-right,bins=28,cellW=plotW/bins;
    ...
    const rows=regionOrder.map((k,ri)=>{const y=top+ri*rowGap,pct=data.length?Math.round(rc[k]/data.length*100):0;const cells=bucketCounts[ri].map((n,i)=>{const x=left+i*cellW+1.2,w=Math.max(2,cellW-2.4),alpha=n?(0.18+0.82*n/maxBucket):0;const from=new Date(tMin+i*tSpan/bins),to=new Date(tMin+(i+1)*tSpan/bins);return '<rect class="region-density '+k.toLowerCase()+'" x="'+x.toFixed(2)+'" y="'+(y-6)+'" width="'+w.toFixed(2)+'" height="12" rx="1" opacity="'+alpha.toFixed(3)+'"><title>'+escSvg(from.toLocaleDateString('zh-CN'))+'–'+escSvg(to.toLocaleDateString('zh-CN'))+' · '+escSvg(REGIONS[k].label)+' · '+n+' 个职位</title></rect>';}).join('');return '<line class="region-time-baseline" x1="'+left+'" y1="'+y+'" x2="'+(left+plotW)+'" y2="'+y+'"/><text class="region-time-name" x="0" y="'+(y+3)+'">'+escSvg(REGIONS[k].label)+'</text><text class="region-time-count" x="'+(TW-1)+'" y="'+(y+3)+'">'+rc[k].toLocaleString()+'<tspan> '+pct+'%</tspan></text>'+cells;}).join('');
