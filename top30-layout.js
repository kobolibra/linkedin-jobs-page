/* Responsive Top 30 layout: real axes, adaptive domains, bounded bubble separation. */
(() => {
  const renderTop30Relaxed = (rows, mode = 'all') => {
    ...
    const maxValue=Math.max(0,...values.flatMap(v=>[v.mean,v.median]));
    const domainMax=Math.max(10,Math.ceil((maxValue*1.12)/5)*5);
    const x=v=>left+(plotRight-left)*((Number(v)||0)/domainMax),y=v=>plotBottom-(plotBottom-plotTop)*((Number(v)||0)/domainMax);
    const tickStep=domainMax<=30?5:domainMax<=80?10:20;
    const ticks=Array.from({length:Math.floor(domainMax/tickStep)+1},(_,i)=>i*tickStep);if(ticks[ticks.length-1]!==domainMax)ticks.push(domainMax);
    const grid=ticks.map(v=>(v===domainMax?'':'<line class="bubble-grid" x1="'+x(v).toFixed(1)+'" y1="'+plotTop+'" x2="'+x(v).toFixed(1)+'" y2="'+plotBottom+'"/>')+'<text class="bubble-axis" x="'+x(v).toFixed(1)+'" y="'+(plotBottom+17)+'" text-anchor="middle">'+v+'</text>').join('')+ticks.map(v=>(v===domainMax?'':'<line class="bubble-grid" x1="'+left+'" y1="'+y(v).toFixed(1)+'" x2="'+plotRight+'" y2="'+y(v).toFixed(1)+'"/>')+'<text class="bubble-axis" x="'+(left-10)+'" y="'+(y(v)+3).toFixed(1)+'" text-anchor="end">'+v+'</text>').join('');
    ...
    for(let pass=0;pass<180;pass++){
      for(let i=0;i<points.length;i++)for(let j=i+1;j<points.length;j++){
        const a=points[i],b=points[j],dx=b.px-a.px,dy=b.py-a.py;let dist=Math.hypot(dx,dy),min=a.r+b.r+5;
        if(dist>=min)continue;if(dist<.01){const angle=((i+1)*37+(j+1)*61)*Math.PI/180;b.px+=Math.cos(angle);b.py+=Math.sin(angle);dist=1;}
        const push=(min-dist)*.5,ux=(b.px-a.px)/dist,uy=(b.py-a.py)/dist;a.px-=ux*push;a.py-=uy*push;b.px+=ux*push;b.py+=uy*push;
      }
    ...
    host.classList.remove('is-ready');host.innerHTML='<svg class="top20-svg bubble-svg" viewBox="0 0 '+W+' '+H+'" role="img" aria-label="Top 30 机构平均首次发现天数、中位数与职位数量气泡图">'+grid+diagonal+'<text class="bubble-x-title" x="'+((left+plotRight)/2)+'" y="'+(H-2)+'" text-anchor="middle">AVG DAYS SINCE FIRST SEEN</text><text class="bubble-y-title" x="31" y="'+((plotTop+plotBottom)/2)+'" text-anchor="middle" transform="rotate(-90 31 '+((plotTop+plotBottom)/2)+')">MEDIAN DAYS</text>'+pointMarkup+'</svg>';requestAnimationFrame(()=>host.classList.add('is-ready'));
  };
